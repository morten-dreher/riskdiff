...
In other words, the treatment with the higher rate is always the favourable one for our examples and we could formulate a null hypothesis as $H_0: \pi_t \leq \pi_c$ when investigating a treatment $t$.
...
However, for small sample sizes, the approximation may be inadequate and one may be interested in exact results.
...
We will now attempt a power calculation and simulation for the same group sizes as above, but assuming the event rates $\pi_t = 0.6$ and $\pi_c = 0.3$.
...
However, this is to be expected, as the normal approximation can always exhaust the full alpha level, whereas the exact risk difference test is slightly overconservative in these cases.
...
The calculation of all possible risk differences for two given group sizes $n_t$ and $n_c$ is straightforward and can be achieved by simply looping through the group sizes.
For each $i \in \{0, \ldots, n_t\}$ and $j \in \{0, \ldots, n_c\}$, the risk difference $\Delta_{\pi, i, j} = i/n_t - j/n_c$ can then be calculated.
Equivalently, $\Delta_{\pi, i, j} = (i \cdot n_c - j \cdot n_t)/(n_t \cdot n_c)$ can be used to avoid possible floating point inaccuracies.
Since the same risk difference may occur for different combinations of $i$ and $j$ and their probabilities must be aggregated, this is the preferred way of calculating the risk differences.
This second representation always leads to a rational number and may also be preferable for manual computations.
