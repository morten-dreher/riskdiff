...
We can compare this power value to that of Fisher's exact test:

```{r simulation_fisher}
fisherReject <- numeric(1e4) # Using fewer iterations

for(i in 1:length(fisherReject)) {
  fisherReject[i] <- fisher.test(matrix(c(successTreatment[i], 4 - successTreatment[i], 
                                         successControl[i], 3 - successControl[i]), nrow = 2),
...
Apart from very early phase testing (in which the focus usually does not lie upon achieving statistical significance), such trial sizes are quite uncommon in practice.
...
These numbers may seem odd, but could very well be the result of a trial attempting to recruit "less odd" numbers, say, 35 and 25 patients per group.
...
```{r comparison_2, cache = TRUE}
piTreatment <- seq(0.2, 0.4, 0.01)
...
```{r plot_comparison_2}
ggplot2::ggplot(data.frame(piTreatment = piTreatment, powerERD = powerERD, powerRpact = powerRpact)) +
  ggplot2::geom_line(ggplot2::aes(x = piTreatment, y = powerERD), col = "blue") +
  ggplot2::geom_line(ggplot2::aes(x = piTreatment, y = powerRpact), col = "red") +
  ggplot2::labs(x = "Treatment success rate", y = "Power", title = "Power of exact risk difference test and normal approximation")
```

```{r summary_comparison_2}
powerERD - powerRpact
summary(powerERD - powerRpact)
```

For larger effect sizes, the exact risk difference test offers a larger power than the normal approximation.
This advantage becomes remarkable for large effect sizes.
...
```{r comparison_5, cache = TRUE}
piTreatment <- seq(0.2, 0.4, 0.01)
powerERD <- numeric(length(piTreatment))

for(i in 1:length(piTreatment)) {
  probs <- riskdiff::driskdiff(nTreatment = 200, nControl = 200, piTreatment = piTreatment[i], piControl = 0.2)
  cumProbs <- riskdiff::priskdiff(nTreatment = 200, nControl = 200, piTreatment = piTreatment[i], piControl = 0.2)
  ...
  powerERD[i] <- rejectData[215, 3]
}

powerRpact <- rpact::getPowerRates(
  pi1 = piTreatment, pi2 = 0.2, maxNumberOfSubjects = 200+200, 
  allocationRatioPlanned = 1, alpha = 0.05
)$overallReject
```
...
For larger effect sizes, the exact risk difference test offers a larger power than the normal approximation, but the gains in power are fairly small.
